...
def register_view(request):
    if request.method == 'POST':
        ...
        if password != password_confirm:
            messages.error(request, 'Las contraseñas no coinciden. Por favor, ingrese nuevamente.')
            return redirect('blog:register')
        
        # Verificar longitud de la contraseña
        if len(password) < 8:
            messages.error(request, 'La contraseña debe tener al menos 8 caracteres. Por favor, ingrese nuevamente.')
            return redirect('blog:register')

        # Verificar si la contraseña contiene al menos una letra y un número
        if not re.search(r'[A-Za-z]', password) or not re.search(r'\d', password):
            messages.error(request, 'La contraseña debe contener al menos una letra y un número. Por favor, ingrese nuevamente.')
            return redirect('blog:register')
        ...
        if User.objects.filter(username=username).exists():
            ...
            print(f'Usuario {username} creado con éxito')  # Mensaje en consola
            login(request, user)  # Loguear automáticamente al usuario
            messages.success(request, f'¡Has creado tu cuenta con éxito, {username}!')
...
def agregar_comentario(request, post_id):
    if request.method == 'POST':
        if not request.user.is_authenticated:
            messages.error(request, "Debes iniciar sesión o registrarte para comentar.")
